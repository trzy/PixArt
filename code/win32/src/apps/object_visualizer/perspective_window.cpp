//TODO: this would be cleaner with class defined in here and a factory method to return i_window

#include "pixart/camera_parameters.hpp"
#include "apps/object_visualizer/perspective_window.hpp"
#include <algorithm>

perspective_window::perspective_window(int width, int height)
...
void perspective_window::init(const pixart::settings &settings)
{
  m_settings = settings;
...
void perspective_window::update(const std::array<PA_object, 16> &objs)
{
...
int perspective_window::distance(const led_position &led1, const PA_object &led2)
{
...
int perspective_window::distance(const led_position &led1, const led_position &led2)
{
...
void perspective_window::sort_by_distance_from(int base_idx, std::array<int, 3> &neighbors_out, const std::array<PA_object, 16> &objs)
{
  // Neighbors of base
  int j = 0;
...
void perspective_window::identify_leds(const std::array<PA_object, 16> &objs)
{
  edge vertical[2];
  edge horizontal[2];
...
int perspective_window::find_shortest_distance(const std::array<led_position, 4> &leds)
{
...
size_t perspective_window::match_to_prior(const std::array<PA_object, 16> &leds, int distance_threshold)
{
...
void perspective_window::canonicalize_leds(const std::array<PA_object, 16> &leds)
{
...
void perspective_window::perspective_update(const std::array<PA_object, 16> &objs)
{
...
void perspective_window::draw_led_board(render::vector3 position, render::euler3 rotation)
{
  using namespace render;
...
void perspective_window::draw_test_scene()
{
  clear();
